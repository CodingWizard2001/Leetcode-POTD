...
public:
    ...
    string find(int num){
        ...
        else if(num<100) result = belowHundred[num/10] + " " + find(num%10);
        ...
        if(size>0 && result[size-1]==' ') result = result.substr(0, size-1);
        return result;
    }
};
...
int main(){
    ...
    int num = 12345;
    cout<<"In english words : "<<solve.numberToWords(num)<<endl;
